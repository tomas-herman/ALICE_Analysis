...
void LuminosityCalculationStandAlone(TString period, TString trigger)
{
  ...
  ULong64_t  class_l0b[nrunsmax] = {0};
  Int_t run;
  Double_t mu = 0;
  ...
  // for (Int_t i=nRunsInList-1;i>=0;i--){
    ...
    cl = (AliTriggerClass*) classes->FindObject("CMUL7-B-NOPF-MUFAST");
    if (!cl) continue;
    Int_t iclassMUL = classes->IndexOf(cl);
    cl = (AliTriggerClass*) classes->FindObject("CMSH7-B-NOPF-MUFAST");
    if (!cl) continue;
    Int_t iclassMSH = classes->IndexOf(cl);
    ...
    Double_t LT_ratio = ((Double_t)class_l2a[iclassMUL]/class_l0b[iclassMUL])/((Double_t)class_l2a[iclassMSH]/class_l0b[iclassMSH]);
    
    LumiFile << srun << ", " << scale*class_lumi[iclass] << "; " << mu <<  "; " << LT_ratio << endl; 

    
    LT_ratioSumWeigted += scale*class_lumi[iclass] * LT_ratio;
    LumiSum += scale*class_lumi[iclass];
  }
  cout << "Live-time uncertainty: " << LT_ratioSumWeigted << "/" << LumiSum << " = " << LT_ratioSumWeigted/LumiSum << endl;
