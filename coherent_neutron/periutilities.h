...
{
  TFile *inFile = TFile::Open(Form("massFitResults/Selection_%i/periData_ZNclass%d_%.2f_y_%.2f_%.2f_pt_%.2f.root",iSel,znSelection,abs(minRap),abs(maxRap),minPt,maxPt));
...
{
  TFile *inFile = TFile::Open(Form("ptFitResults/Selection_%i/periData_ZNclass%d_%.2f_y_%.2f_%.2f_mass_%.2f.root",iSel,znSelection,abs(minRap),abs(maxRap),minMass,maxMass));
