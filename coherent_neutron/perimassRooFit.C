...
{
  TFile *inFile = TFile::Open(Form("massMcParameters/perimcParam_data_%i_%.2f_y_%.2f_%.2f_pt_%.2f.root",iData,abs(minRap),abs(maxRap),minPt,maxPt));
        ...
        double minRap, double maxRap)
{
  ...
  TFile *fOut = new TFile(Form("massMcParameters/perimcParam_data_%i_%.2f_y_%.2f_%.2f_pt_%.2f.root",iData,abs(minRap),abs(maxRap),minPt,maxPt),"recreate");
		    ...
		    double minRap, double maxRap)
{
  ...
  mass.setRange("m_JPsiRange",2.85, 3.35);
  RooRealVar rap("rap","rap",minRap,maxRap);
  RooRealVar znclass("znclass","znclass",0,4);
  ...
  leg->SetBorderSize(0);
  leg->SetTextSize(0.038);
  if (znSelection == -2) leg->AddEntry((TObject*)0, "all", "");
  ...
  //---Save as pdf
  char massFit[120];
  sprintf(massFit,"MassPlots/Selection_%i/periMass_fit_%.2f_y_%.2f_%.2f_pt_%.2f_ZNclass_%d.png", iSel, minRap,maxRap,minPt,maxPt,znSelection);
  c1->SaveAs(massFit);
  cout << "chi^2 = " << frame->chiSquare() << endl;
  cout << " Entries " << inData.numEntries() << endl;

  // set up the output file to save the fit results
  TFile *fOut = new TFile(Form("massFitResults/Selection_%i/periData_ZNclass%d_%.2f_y_%.2f_%.2f_pt_%.2f.root",iSel,znSelection,abs(minRap),abs(maxRap),minPt,maxPt),"recreate");
...
void perimassRooFit(int iData = 0, int iSel = 1, int znSelection = -2)
{
