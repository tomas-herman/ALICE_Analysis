...
void LuminosityCalculationStandAlone(TString period, TString trigger)
{
  ...
  if (period.Contains("18q")){
    if (trigger.Contains("CMUP6")){
      sprintf(FilePath,"AnalysisResultsLHC18qr_ptCoherent.root");
      sprintf(FileNameTXT,"Lumi/18q_lumi_CMUP6.txt");
    } else if (trigger.Contains("CMUP11")){
      sprintf(FilePath,"/mnt/data/Data_processing/Processed_data/TrainResults/567_20200508-1942-Data/LHC18q_AnalysisResults.root");
    ...
    if (trigger.Contains("CMUP6")){
      ...
      sprintf(FilePath,"/mnt/data/Data_processing/Processed_data/TrainResults/567_20200508-1942-Data/LHC18r_AnalysisResults.root");
      sprintf(FileNameTXT,"Lumi/18r_lumi_CMUP11.txt");    
    }  
  } else if (period.Contains("15o")){
    if (trigger.Contains("CMUP10")){
      sprintf(FilePath,"/mnt/data/Data_processing/Processed_data/TrainResults/567_20200508-1942-Data/LHC15o_AnalysisResults.root");
  ...
  TFile *TrgFile= new TFile(FilePath); 
   // TFile *TrgFile= new TFile("AnalysisResultsLHC18qr_ptCoherent.root"); 
  gDirectory->cd("MyTask");
  TList* list = (TList*) gDirectory->Get("ADcheck_");

  TH2F * hist = (TH2F*) list->FindObject("fTriggersVsRunH");

  for (Int_t irun=0;irun<nRunsInList;irun++){
    if (trigger.Contains("CMUP6")){
      hCMUPtrg->SetBinContent(irun+1, hist->GetBinContent(3,runList[irun]-240000+1));
    }        
    // cout << runList[irun] << "  " << hist->GetBinContent(3,runList[irun]-240000+1) << endl;
  ...
  // for (Int_t i=nRunsInList-1;i>=0;i--){
    ...
    LumiFile << srun << ", " << scale*class_lumi[iclass] <<  endl; 
  }
  LumiFile.close();
