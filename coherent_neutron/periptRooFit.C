                        		     ...
                        		     double minRap, double maxRap)
{
  ...
  RooDataHist dataHistTwoGammaToMuMedium("dataHistTwoGammaToMuMedium","dataHistTwoGammaToMuMedium",pt,getPtTemplateHisto(-1, 0, -1, -1),1); // Data driven PDF
  ...
  pt.setRange("coherentPtRange",0.0,0.3);
  // ---Create integrals for pt < 0.3
  ...
  if (znSelection == -2) leg2->AddEntry((TObject*)0, "all", "");
  ...
  //---Save as pdf
  char ptFit[120];
  sprintf(ptFit,"ptPlots/Selection_%i/periFixpt_fit_%.2f_y_%.2f_%.2f_mass_%.2f_ZNclass_%d.png", iSel, minRap,maxRap,minMass,maxMass,znSelection);
  c1->SaveAs(ptFit);
  cout << "chi^2 = " << frame->chiSquare() << endl;
  cout << " Entries " << inData.numEntries() << endl;

  // set up the output file to save the fit results
  TFile *fOut = new TFile(Form("ptFitResults/Selection_%i/periData_ZNclass%d_%.2f_y_%.2f_%.2f_mass_%.2f.root",iSel,znSelection,abs(minRap),abs(maxRap),minMass,maxMass),"recreate");
...
void periptRooFit(int iData = 0, int iSel = 1)
{
